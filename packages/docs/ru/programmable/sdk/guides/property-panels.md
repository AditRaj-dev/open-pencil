...
description: Создание панелей свойств с помощью composables и headless components для списков.
---
...
Если панели нужны вычисленные из выделения значения и действия для их изменения, используйте composable. Если важна переиспользуемая структура массива или списка, используйте headless component, например `PropertyListRoot`.
...
Если значение поля можно связать с переменной или внешним design token, оберните поле в `BindableValueRoot`. Компонент не задаёт внешний вид поля, однако интерфейс не должен разрушать существующую привязку при простом получении focus:

- Пока поле не редактируют, показывайте имя переменной. Вычисленное значение можно вывести во вспомогательном элементе, например в tooltip.
- Получение focus и открытие выбора переменной не должны удалять привязку.
...
- Замену привязки, её удаление при редактировании и изменение нескольких объектов выполняйте в одной batch operation provider.

В интерфейсе приложения OpenPencil связанное поле в обычном состоянии показывает имя переменной на фиолетовом фоне. После начала редактирования `NumberField` показывает вычисленное числовое значение. В собственном интерфейсе редактора то же headless-состояние можно представить иначе.
...
- Используйте структурные headless components, когда основная сложность заключается в координации повторяющихся списков, деревьев или slots.
