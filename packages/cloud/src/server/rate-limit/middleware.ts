...
export function createTrustedIPRateLimiter(
  ...
  return createCloudRateLimiter({
    ...
    secret,
    policy,
    keyGenerator: (context) => trustedClientIP(context, client) ?? 'unavailable',
    skip: (context) => trustedClientIP(context, client) === null,
    handler
  })
}
...
export function createActorRateLimiter(
  ...
  return createCloudRateLimiter<CloudAPIEnvironment, '*'>({
    ...
    secret,
    policy,
    keyGenerator: (context) => `actor:${context.get('actor').userId}`,
    skip: skip ? (context) => skip(context.req.method) : undefined
  })
}
