...
async function writeModernBrowserClipboard(html: string, plainText: string): Promise<boolean> {
  if (
...
function writeLegacyBrowserClipboard(html: string, plainText: string): boolean {
  if (
...
export async function copySelectionToBrowserClipboard(store: EditorStore): Promise<boolean> {
  try {
    ...
    if (await writeModernBrowserClipboard(html, plainText)) return true
    return writeLegacyBrowserClipboard(html, plainText)
  } catch (error) {
