...
When adding user-facing changes, update `CHANGELOG.md` under the single appropriate `Unreleased` category. Treat it as curated release notes, not a commit log: omit tests, benchmarks, CI, internal refactors, development tooling, and bugs introduced and fixed since the last release. After merges, review the complete `Unreleased` section against changes since the latest release, consolidate related work into end-user outcomes, preserve important shipped-facing changes, and remove duplicate bullets or category headings. Use the public categories `Breaking changes`, `Added`, `Changed`, `Fixed`, `Performance`, and `Security`; omit empty categories. Use `## x.y.z — YYYY-MM-DD` for release headings, write one present-tense user-visible outcome per bullet, end complete sentences with periods, and append relevant issue or PR references such as `(#395)`. Update `README.md` when appropriate for user-facing features and `AGENTS.md` when architecture or conventions change. Do not put speculative/internal implementation plans in `packages/docs/**`; VitePress docs are published. Keep temporary plans in ignored `scratch/` or distill durable public direction into the canonical roadmap.
